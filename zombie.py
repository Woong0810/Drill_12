...
class Zombie:
    ...
    def distance_less_than(self, x1, y1, x2, y2, r):
        # 제곱근을 쓰지않고 그냥 제곱끼리의 대소 비교로 처리한다.
    ...
    def move_to_boy(self, r=0.5):
        ...
        # 소년에 근접했으면 성공 리턴
        if self.distance_less_than(common.boy.x, common.boy.y, self.tx, self.ty, r):
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.RUNNING
    ...
    def zombie_has_more_ball_than_boy(self):
        if self.ball_count >= common.boy.ball_count:
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.FAIL

    def zombie_has_less_ball_than_boy(self):
        if self.ball_count < common.boy.ball_count:
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.FAIL
